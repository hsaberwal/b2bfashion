...
type Order = {
  id: string;
  items: { productId: string; sku: string; quantity: number; pricePerPiece?: number; packSize?: number; size?: string }[];
  status: string;
};
...
export default function SignOrderPage() {
  ...
  const orderTotal = order?.items?.reduce((sum, i) => sum + (i.pricePerPiece ?? 0) * i.quantity, 0) ?? 0;
