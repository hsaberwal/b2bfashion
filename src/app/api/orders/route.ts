...
function mapOrder(o: {
  _id: unknown;
  items: { productId: unknown; sku: string; quantity: number; pricePerPiece?: number; packSize?: number; size?: string }[];
  status: string;
  signedAt?: Date;
  createdAt: Date;
}) {
...
export async function GET() {
  try {
    ...
    if (cart && pricingApproved) {
      const items = (cart.items ?? []) as { productId: unknown; sku: string; quantity: number; pricePerPiece?: number; packSize?: number; size?: string }[];
      const productIds = items.map((i) => i.productId);
      const products = await Product.find({ _id: { $in: productIds } }).select("pricePerPiece pricePerPack").lean();
      const priceMap = new Map(
        products.map((p) => {
          const pp = p as { pricePerPiece?: number; pricePerPack?: number };
          return [String(p._id), pp.pricePerPiece ?? pp.pricePerPack];
        })
      );
      for (const item of items) {
        const currentPrice = priceMap.get(String(item.productId));
        if (currentPrice !== undefined) {
          item.pricePerPiece = currentPrice;
        }
      }
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    const newLines: { productId: string; sku: string; quantity: number; pricePerPiece?: number; packSize: number }[] = [];
    for (const item of items) {
      ...
      newLines.push({
        ...
        pricePerPiece: (user.role === "admin" || user.pricingApproved) ? (product.pricePerPiece ?? product.pricePerPack) : undefined,
    ...
    const existing = order.items as { productId: mongoose.Types.ObjectId; sku: string; quantity: number; pricePerPiece?: number; packSize?: number }[];
    const byKey = new Map<string, { productId: mongoose.Types.ObjectId; sku: string; quantity: number; pricePerPiece?: number; packSize: number }>();
    ...
    for (const line of newLines) {
      ...
      if (current) {
        ...
        current.pricePerPiece = line.pricePerPiece;
      } else {
