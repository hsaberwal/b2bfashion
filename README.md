...
- **Sales agents** (field reps): an `agent` role + admin **Agents** section (invite agents, assign customers), plus an **agent portal** (`/agent`) where a rep picks a customer, builds a basket, the customer signs on the device, and the order is placed with any enabled payment option (unpaid invoice or Stripe card/Apple Pay/Klarna). Barcode scanning of sample packs lands in the next phase.
